...
export function ColourExplorer() {
  ...
  return (
    <section className="min-h-[90vh] bg-canvas px-6 py-section-y md:px-section-x">
      <div className="grid grid-cols-1 gap-16 lg:grid-cols-[45%_55%] lg:gap-20">
        ...
        <div className="lg:sticky lg:top-[100px] lg:h-fit">
          <Reveal>
            ...
            <h2 className="mt-4 font-display text-[clamp(38px,5vw,64px)] font-bold leading-[0.9] text-ivory-text">
              2,300 shades<span className="text-orange">.</span>
            </h2>
            <p className="mt-6 max-w-[320px] font-sans text-body leading-[1.7] text-muted">
              ...
              {FAMILY_CHIPS.map((chip) => (
                <span key={chip.name} className="group relative">
                  <Link
                    ...
                    className="swatch block h-8 w-8 rounded-full border-2 border-transparent transition-colors duration-200 hover:border-ivory-text"
                    style={{ background: chip.hex }}
                  />
                  <span className="pointer-events-none absolute -top-9 left-1/2 z-10 -translate-x-1/2 whitespace-nowrap rounded-full bg-card px-3 py-1 font-sans text-[11px] font-medium text-ivory-text opacity-0 transition-opacity duration-200 group-hover:opacity-100">
                    {chip.name}
                  </span>
                </span>
              ))}
